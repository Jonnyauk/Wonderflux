		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h4><?php _e( 'Group Forums', 'wonderflux' ); ?></h4>

					<?php if ( bp_forums_is_installed_correctly() ) : ?>

						<p><?php _e( 'Should this group have a forum?', 'wonderflux' ); ?></p>

						<div class="checkbox">
							<label for="group-show-forum"><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php _e( 'Enable discussion forum', 'wonderflux' ); ?></label>
						</div>
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php printf( __( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'wonderflux' ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) ); ?></p>
			...
			<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
				...
				<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<div class="main-column">
						...
						<p><?php _e( 'To skip the group profile photo upload process, hit the "Next Step" button.', 'wonderflux' ); ?></p>
					</div><!-- .main-column -->
				...
				<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<h4><?php _e( 'Crop Group Profile Photo', 'wonderflux' ); ?></h4>
			...
			<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
				...
				<?php if ( bp_is_active( 'friends' ) && bp_get_total_friend_count( bp_loggedin_user_id() ) ) : ?>
					...
					<div class="main-column">
						...
						<div id="message" class="info">
							<p><?php _e('Select people to invite from your friends list.', 'wonderflux' ); ?></p>
						</div>
						...
						<?php if ( bp_group_has_invites() ) : ?>
							...
							<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
								...
								<li id="<?php bp_group_invite_item_id(); ?>">
									...
									<div class="action">
										<a class="remove" href="<?php bp_group_invite_user_remove_invite_url(); ?>" id="<?php bp_group_invite_item_id(); ?>"><?php _e( 'Remove Invite', 'wonderflux' ); ?></a>
									</div>
								</li>
				...
				<?php else : ?>
					...
					<div id="message" class="info">
						<p><?php _e( 'Once you have built up friend connections you will be able to invite others to your group.', 'wonderflux' ); ?></p>
					</div>
			...
			<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
				...
				<div class="submit" id="previous-next">
					...
					<?php if ( !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="button" value="<?php esc_attr_e( 'Back to Previous Step', 'wonderflux' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
					...
					<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Next Step', 'wonderflux' ); ?>" id="group-creation-next" name="save" />
					...
					<?php if ( bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Create Group and Continue', 'wonderflux' ); ?>" id="group-creation-create" name="save" />
					...
					<?php if ( bp_is_last_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Finish', 'wonderflux' ); ?>" id="group-creation-finish" name="save" />
